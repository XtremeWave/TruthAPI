...
using TruthAPI.Modules.Runtime;
...
public static class RPCRouter
{
    public class RPCSection : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            currentSection = null;
            //Info(NebulaLog.LogLevel.Log, $"End Evacuating Rpcs ({Name}, Size = {evacuateds.Count})");
        ...
        public RPCSection(string? name = null)
        {
            ...
            {
                currentSection = this;
                //Info(NebulaLog.LogLevel.Log,  $"Start Evacuating Rpcs ({Name})");
            }
            else
            {
                //Info(NebulaLog.LogLevel.Log, $"Rpc section \"{Name}\" is in \"{currentSection.Name}\"! It is ignored.");
            }
        }
    }
    ...
    public static void SendRpc(string name, int hash, Action<MessageWriter> sender, Action localBodyProcess, bool shouldBeReliable)
    {
        if (currentSection == null)
        {
            ...
            writer.Write(hash);
            sender.Invoke(writer);
            //Info("sent RPC:" + name + "(size:" + writer.Length + ")");
            ...
            {
                Info($"Error in RPC(Invoke: {name})\n" + ex.ToString(), "SendRpc");
            }

            //Info(NebulaLog.LogLevel.Log, $"Called RPC : {name}");
        }
        else
        {
            evacuateds.Add(new(hash, sender, localBodyProcess));

            //Info(NebulaLog.LogLevel.Log, $"Evacuated RPC : {name} (by {currentSection!.Name})");
        }
    }
}
...
public class RemoteProcessBase
{
    ...
    public RemoteProcessBase(string name, bool shouldBeReliable)
    {
        ...
        if (AllNebulaProcess.ContainsKey(Hash)) Info(name + " is duplicated. (" + Hash + ")", "RemoteProcessBase");
        else Info(name + " is registered. (" + Hash + ")", "RemoteProcessBase");
    ...
    static System.Collections.IEnumerator Preprocess(NebulaPreprocessor preprocessor)
    {
        ...
        foreach (var type in types)
        {
            ...
            foreach (var method in type.GetMethods().Where(m => m.IsDefined(typeof(NebulaRPC)))) WrapRpcMethod(TruthAPI.TruthAPI.Harmony, method);
        }
    }
...
public class CombinedRemoteProcess : RemoteProcessBase
{
    ...
    public override void Receive(MessageReader reader)
    {
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            {
                Info("RPC NotFound ID Error. id: " + id + " ,index: " + i + " ,length: " + num, "Receive");
...
public static class RoleRPC
{
    ...
    static private int RegisterAction<Ability>(string id, Action<Ability, int, bool> action, Predicate<Ability>? predicate) where Ability : IPlayerAbility
    {
        ...
        if (actions.ContainsKey(hash)) Info("Duplicated role argument sync message: " + id + " (Hash: " + hash + ")", "RegisterAction");
        actions[hash] = (num, player, calledByMe) => ActionInternal<Ability>(player, num, action, predicate, calledByMe);
        Info("Registered role argument sync message: " + id + " (Hash: " + hash + ")", "RegisterAction");
        return hash;
    }
    ...
    static private readonly RemoteProcess<(int id, int num, GamePlayer player)> RpcSyncArgument = new("SyncRoleArgument", (message, calledByMe) =>
    {
        ...
        {
            Info("Unknown role argument sync message: " + message.id, "RemoteProcess");
        }
    });
...
class NebulaRPCInGameHandlerPatch
{
    static public void ReceiveMessage(MessageReader reader)
    {
        ...
        {
            Info("RPC NotFound Error. id: " + id, "ReceiveMessage");
