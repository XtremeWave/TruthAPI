...
using System.Text;
using TruthAPI.Enums;
using TruthAPI.Managers.UpdateTools;
...
namespace TruthAPI.Managers
{
    public static class UpdateManager
    {
        ...
        /// <summary>
        /// Note: TruthAPI's update detection determines whether there is an update by checking the `tag_name` of the latest release on GitHub and the Assembly version of the mod. The `tag_name` should be written as "v1.0.0" or "1.0.0".
        /// </summary>
        ...
        public static void CheckForUpdates()
        {
            try
            {
                ...
                {
                    if (TruthAPI.Logging)
                        TruthAPI.Logger.LogInfo("Checking for updates skipped.");
                    return;
                }
                
                if (TruthAPI.Logging)
                    TruthAPI.Logger.LogInfo("Checking for updates..");
                ...
                {
                    if (TruthAPI.Logging)
                        TruthAPI.Logger.LogInfo("All mods are up-to-dated..");
                    return;
                }
                ...
                foreach (var updateListener in updateListeners)
                {
                    ...
                    if (TruthAPI.Logging)
                        TruthAPI.Logger.LogInfo($"Found an update for {updateListener.Name}!");
                }
