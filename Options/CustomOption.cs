    ...
    public static void SaveSettings()
    {
        ...
        try
        {
            ...
            foreach (var option in AllOptions)
            {
                ...
                {
                    var valueEntry = TruthAPI.ConfigFile.Bind("Settings", $"{safeKey}_Value",
                        option.DefaultValue?.ToString(),
                        $"Custom option: {option.Name} (Value)");

                    var value2Entry = TruthAPI.ConfigFile.Bind("Settings", $"{safeKey}_Value2",
                ...
                {
                    var valueEntry = TruthAPI.ConfigFile.Bind("Settings", safeKey,
                        option.DefaultValue?.ToString(),
